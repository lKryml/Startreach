...
import type { IProjects } from "@/_interfaces/projects.interface"
...
export const find = async (pagination: IPagination): Promise<IResponse<IProjects[]> | null> => {
	return http.getAll<IProjects>(prepareSearchPagination(_controller, pagination))
}

export const destroy = async (itemId: IProjects["id"]) => http.delete(_controller, itemId)
