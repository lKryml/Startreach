...
import type { IPagination, IResponse } from "@/interfaces";
...
export class HttpClient {
	...
	private async fetchData<T>(url: string, method: string, data?: any): Promise<IResponse<T> | null> {
	...
	// Update
	async update<T>(resource: string, id: number | string, data: any): Promise<IResponse<T> | null> {
		return await this.fetchData<IResponse<T>>(`${resource}/${id}`, "PUT", data) as IResponse<T>
	}
	...
	public getQueryParams(params: IPagination): string {
		if (params) {
			return Object.keys(params).map(key => `${key}=${(<any>params)[key]}`).join("&");
		} else return '';
	}
}
